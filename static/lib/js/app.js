...
define(["require", "exports"], function (require, exports) {
    "use strict";
    var App = (function () {
        function App(appName, version) {
            this.appName = appName;
            this.version = version;
        }
        App.prototype.println = function () {
            console.log(this.appName, this.version, new Date());
        };
        App.prototype.alert = function (msg, func) {
            var self = this;
        ...
        App.prototype.confirm = function (msg, yesFunc, noFunc) {
            var _this = this;
            ...
            div.css('left', Math.floor((winW - w) / 2)).css('top', Math.floor((winH - h) / 2));
            div.fadeIn();
            $(this.confirmElement).find('.btn-gray').get(0).onclick = function (event) {
                $(_this.confirmElement).fadeOut();
                if (noFunc) {
                    noFunc();
                }
            };
            $(this.confirmElement).find('.btn-yellow').get(0).onclick = function (event) {
                $(_this.confirmElement).fadeOut();
                if (yesFunc) {
                    yesFunc();
                }
        ...
        App.prototype.confirm2 = function (msg, func) {
            var self = this;
            if (!this.toastElement) {
                this.toastElement = document.createElement('div');
                this.toastElement.className = 'dialog';
        ...
        App.prototype.validateNumber = function (str) {
            if (!(str).match(/^\d+(\.\d+)?$/))
                return false;
            return true;
        };
        App.prototype.validateMobile = function (str) {
            if (!(str).match(/^[1][3|4|5|7|8][0-9]{9}$/))
                return false;
            return true;
        };
        App.prototype.validateLoginName = function (str) {
            if (!(str).match(/^[a-zA-Z_][0-9a-zA-Z_]{3,15}$/))
                return false;
            return true;
        };
        App.prototype.validateEmail = function (str) {
            if (!str || !str.length)
                return false;
            if (!('' + str).match(/[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+/))
                return false;
            return true;
        };
        /**
         ...
         * @param {Int} day
         */
        App.prototype.setCookie = function (name, value, day) {
            var Days = day || 30;
            var exp = new Date();
         ...
         * @returns {boolean}
         */
        App.prototype.isLocalStorageSupported = function () {
         ...
         * @param {Object} value
         */
        App.prototype.setLocalStorage = function (name, value) {
            if (!this.isLocalStorageSupported())
        ...
        App.prototype.getURLParameter = function (name, win) {
        ...
        App.prototype.validateIdNumber = function (str) {
